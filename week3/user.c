...
void addUser(user data)
{
    ...
    if(head == NULL)
    {
        ...
        strcpy(head->data.param, data.param);
        head->next = NULL;
    }
    else
    {
        ...
        strcpy(newUser->data.param, data.param);
        newUser->next = NULL;
    }
}
...
void readFile()
{
    ...
    {
        while(fscanf(fptr, "%s %s %d %s", data.username, data.password, &data.status, data.param) != EOF)
        {
            addUser(data);
        }
        fclose(fptr);
...
void writeFile()
{
    ...
    else
    {
        ...
        while(tmp != NULL)
        {
            fprintf(fptr, "%s %s %d %s\n", tmp->data.username, tmp->data.password, tmp->data.status, tmp->data.param);
            tmp = tmp->next;
        }
    }
    fclose(fptr);
...
void convertIP()
{
    ...
    while(tmp != NULL)
    {
        if(strcmp(tmp->data.username, currentUser) == 0)
        {
            strcpy(data.param, tmp->data.param);
            break;
        }
        tmp = tmp->next;
    }
    printf("IP address: %s\n", data.param);
    printf("\nConverting ... \n\n");
    addr.s_addr = inet_addr(data.param);
